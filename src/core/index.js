...
module.exports = {
	...
	throw: function(msg='Unknown', err){
		if (process.env.NODE_ENV !== 'production' && process.stderr.isTTY)
			msg = Chalk.red(msg);
		...
		// TODO: Validations for FaiError
		if (this.throw.FaiError) throw new this.throw.FaiError(msg);
		throw new Error(msg);
	},
	...
	use: function(callee, conf={}, attr={}){
		...
		if (!mod.name)
			this.throw(msg, new ReferenceError('Expected a named function.'));
